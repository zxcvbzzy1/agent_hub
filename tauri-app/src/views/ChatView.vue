...
import { deliverBlob, deliverUrl } from '@/desktop/download'
import { pickLocalFiles } from '@/desktop/files'
...
// ── 桌面端：本地文件发送到对话 ─────────────────────────────
// 选中/拖入的文件先上传成 artifact，再以 markdown 链接形式插进输入框，随消息一起发出。
const attaching = ref(false)

async function uploadLocalFileEntries(files) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const url = `${API_BASE_URL}/api/im/artifacts/${r.item.artifact_id}`
      links.push(file.contentType?.startsWith('image/') ? `![${file.name}](${url})` : `[${file.name}](${url})`)
    }
    const prefix = composer.value && !composer.value.endsWith('\n') ? '\n' : ''
    composer.value += `${prefix}${links.join('\n')}\n`
    message.success(`已附加 ${files.length} 个本地文件`)
    nextTick(() => composerRef.value?.focus?.())
  } catch (error) {
    message.error('本地文件上传失败')
  } finally {
...
async function attachLocalFiles() {
  ...
  await uploadLocalFileEntries(files)
}

let unlistenDragDrop = null

async function setupDesktopDragDrop() {
  if (!isTauri) return
  const { getCurrentWebview } = await import('@tauri-apps/api/webview')
  const { readLocalFiles } = await import('@/desktop/files')
  unlistenDragDrop = await getCurrentWebview().onDragDropEvent(async (event) => {
    if (event.payload?.type !== 'drop' || !event.payload.paths?.length) return
    try {
      const files = await readLocalFiles(event.payload.paths)
      await uploadLocalFileEntries(files)
    } catch {
      message.error('读取拖入的文件失败')
    }
  })
}
...
// 桌面端：系统保存对话框落盘 + Finder 显示；浏览器：anchor 下载（兼容逻辑在 deliver* 内部）。
function triggerBlobDownload(blob, filename) {
  deliverBlob(blob, filename)
}

function triggerUrlDownload(fileUrl, filename) {
  deliverUrl(fileUrl, filename)
}
      ...
      <footer class="composer">
        ...
        <div class="composer-actions">
          ...
          <a-button
            v-if="isTauri"
            :loading="attaching"
            title="发送本地文件（也可以直接把文件拖进窗口）"
            @click="attachLocalFiles"
          >

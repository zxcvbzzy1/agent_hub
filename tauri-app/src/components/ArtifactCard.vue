...
async function downloadDeploy() {
  ...
  try {
    ...
    await deliverBlob(blob, `${(props.artifact?.title || 'deploy').replace(/\s+/g, '_')}.zip`)
  } catch {
    message.error('下载失败')
  }
}
...
function downloadArtifact() {
  ...
  if (type === 'document') {
    ...
    deliverBlob(blob, base.includes('.') ? base : `${base}.${ext}`)
  } else if (type === 'message') {
    const blob = new Blob([artifact.content || ''], { type: 'text/plain;charset=utf-8' })
    const base = (artifact.title || 'message').replace(/\s+/g, '_')
    deliverBlob(blob, `${base}.txt`)
  } else if (type === 'diff') {
    ...
    deliverBlob(blob, base.includes('.') ? base : `${base}.diff`)
  } else if (type === 'web') {
    if (artifact.html) {
      const blob = new Blob([artifact.html], { type: 'text/html;charset=utf-8' })
      const base = (artifact.title || 'web').replace(/\s+/g, '_')
      deliverBlob(blob, base.includes('.') ? base : `${base}.html`)
    } else if (artifact.url) {
      deliverUrl(artifact.url, (artifact.title || 'web').replace(/\s+/g, '_'))
    }
  } else if (type === 'image') {
    if (artifact.url) {
      const urlBasename = artifact.url.split('/').pop()?.split('?')[0] || 'image'
      deliverUrl(artifact.url, (artifact.title || urlBasename || 'image').replace(/\s+/g, '_'))
    }
  }
}
</script>
